...
public class ComprasPorRangoFechaPrecio implements Query {
	...
	public Object Execute(Object params) {
		...
		return buscarComprasPorRangoFechaPrecio((int) args.get("desdePrecio"), (int) args.get("hastaPrecio"),(LocalDate) args.get("desdeFecha"),(LocalDate) args.get("hastaFecha"));
	}
	
	public List<TCompra> buscarComprasPorRangoFechaPrecio(int desdePrecio, int hastaPrecio,LocalDate desdeFecha, LocalDate hastaFecha) {
		...
		try {
			...
			ResultSet r = s.executeQuery(
					"SELECT * FROM COMPRA WHERE ACTIVO = 1 " + " AND PRECIO_TOTAL BETWEEN " + desdePrecio + " AND "
							+ hastaPrecio+
							" AND FECHA >= '" + desdeFecha.toString() + "' AND FECHA <='" + hastaFecha.toString() + "'"+ slctforUpdate +  " ;"); 
