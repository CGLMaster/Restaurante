	...
	public List<TProducto> buscarProductosPorRangoPrecio(int desde, int hasta) {
		...
		try {
			...
			ResultSet r = s.executeQuery(
					"SELECT * FROM PERECEDERO PE JOIN PRODUCTO PR ON PR.ID_PRODUCTO=PE.ID_PRODUCTO WHERE PR.ACTIVO = 1 " + " AND PRECIO BETWEEN " + desde + " AND " + hasta + slctforUpdate + " ;");
			...
			r = s.executeQuery(
					"SELECT * FROM NO_PERECEDERO NP JOIN PRODUCTO PR ON PR.ID_PRODUCTO=NP.ID_PRODUCTO WHERE PR.ACTIVO = 1 " + " AND PRECIO BETWEEN " + desde + " AND " + hasta  + slctforUpdate + " ;");
			while (r.next()) {
				int id = r.getInt("ID_PRODUCTO");
				float precio = r.getFloat("PRECIO");
				int stock = r.getInt("STOCK");
