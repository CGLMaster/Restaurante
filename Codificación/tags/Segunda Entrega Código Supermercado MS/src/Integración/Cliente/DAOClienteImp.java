...
public class DAOClienteImp implements DAOCliente {
	...
	public int altaCliente(TCliente cliente) {
		...
		try {
			...
			exito = s.executeUpdate("INSERT INTO CLIENTES (NOMBRE, DNI, MAIL, ACTIVO) VALUES ('"+ cliente.getNombre() + "', '" 
			+ cliente.getDNI() + "', '" + cliente.getMail() + "', '1');", Statement.RETURN_GENERATED_KEYS);
			...
			if(idInsertado.next()){
				...
				if (cliente instanceof TClientePremium) {
					...
					exito = s.executeUpdate("INSERT INTO CLIENTE_PREMIUM (ID_SOCIO, ANTIGUEDAD,TELEFONO,DIRECCION) VALUES ("
							+ idCliente + ",'" + clienteP.getAntiguedad().toString() + "'," + clienteP.getTelefono() + ",'"
							+ clienteP.getDireccion() + "');");
				} else if (cliente instanceof TClienteNoPremium) {
					exito = s.executeUpdate("INSERT INTO CLIENTE_NO_PREMIUM (ID_SOCIO, N_COMPRAS) VALUES (" + idCliente + "," + "'0');");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public TCliente buscarUnCliente(int id) {
		...
		try {
			...
			if (resultSet != null && resultSet.first() ) {
				...
				resultSet = s.executeQuery(
					"SELECT C.ID_SOCIO, C.NOMBRE, C.DNI, C.MAIL, C.ACTIVO, CNP.N_COMPRAS"
	...
	public int buscarUnClientePorDni(String dni) {
		...
		try {
			...
			if (resultSet != null && resultSet.first() ) salida = resultSet.getInt("ID_SOCIO");
	...
	public List<TCliente> buscarTodosCliente() {
		...
		try {
			...
			resultSet = s.executeQuery(
					"SELECT * FROM CLIENTE_PREMIUM JOIN CLIENTES ON CLIENTES.ID_SOCIO = CLIENTE_PREMIUM.ID_SOCIO WHERE ACTIVO = 1" + slctforUpdate + ";");
			...
			while(resultSet.next()){
				...
				resultSet = s.executeQuery(
						"SELECT * FROM CLIENTE_NO_PREMIUM JOIN CLIENTES ON CLIENTES.ID_SOCIO = CLIENTE_NO_PREMIUM.ID_SOCIO WHERE ACTIVO = 1" + slctforUpdate + ";");
				while(resultSet.next()){
