...
public class DAOSeccionImp implements DAOSeccion {
	...
	public int altaSeccion(TSeccion seccion) {
		...
		try {
			...
			exito = s.executeUpdate("INSERT INTO SECCION (ZONA,PASILLO,CONT_PRODUCTOS,ACTIVO) VALUES ('"
					+ seccion.getZona() + "'," + seccion.getPasillo() + ","
					+ seccion.getCont_productos() + ",true);", Statement.RETURN_GENERATED_KEYS);
	...
	public int buscarSeccionPorZonaPasillo(String zona, int pasillo) {
		...
		try {
			...
			resultSet = s.executeQuery("SELECT ID_SECCION FROM SECCION WHERE ZONA = '" + zona + "' AND PASILLO = " + pasillo + slctforUpdate +";");
	...
	public int modificarSeccion(TSeccion seccion) {
		...
		try {
			...
			exito = s.executeUpdate("UPDATE SECCION SET PASILLO = '" + seccion.getPasillo() + "', ZONA = '"
					+ seccion.getZona() +"', CONT_PRODUCTOS = "+ seccion.getCont_productos()+ " WHERE ID_SECCION = " + seccion.getID_Seccion()+ ";");
