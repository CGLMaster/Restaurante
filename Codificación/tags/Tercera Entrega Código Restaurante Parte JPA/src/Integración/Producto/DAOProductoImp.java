	...
	public int altaProducto(TProducto p) {
		...
		try {
			...
			if (p instanceof TProdNoPerecedero) {
				...
				exito = s.executeUpdate("INSERT INTO PERECEDERO (ID_PRODUCTO,FECHA_DE_CADUCIDAD) " + "VALUES(" + exito
						+ ",'" + perec.getFechaDeCaducidad().toString() + "');");
			}
			exito = id;
	...
	public int modificarProducto(TProducto prod) {
		try {
			...
			if (prod instanceof TProdNoPerecedero) {
				res = s.executeUpdate("UPDATE NO_PERECEDERO SET TIPO = '" + ((TProdNoPerecedero) prod).getTipo()
						+ "' WHERE ID_PRODUCTO = " + prod.getIDProducto() + " ;");

			} else if (prod instanceof TProdPerecedero) {
				res = s.executeUpdate("UPDATE PERECEDERO SET FECHA_DE_CADUCIDAD = '"
						+ ((TProdPerecedero) prod).getFechaDeCaducidad().toString() + "' WHERE ID_PRODUCTO = "
						+ prod.getIDProducto() + " ;");
			}
			...
			if (r.next()) {
				...
				if (r2.next()) {
					p = new TProdNoPerecedero(r2.getInt("ID_PRODUCTO"), r2.getString("NOMBRE"), r2.getInt("STOCK"),
							r2.getFloat("PRECIO"), r2.getInt("ID_MARCA"), r2.getInt("ID_SECCION"),
							r2.getBoolean("ACTIVO"), r2.getString("TIPO"));
				}
			}
